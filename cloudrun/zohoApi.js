...
/**
 * Get an estimate from Zoho Books
 * @param {string} estimateId - The Zoho estimate ID
 * @returns {Promise<Object>} The Zoho API response
 */
async function getEstimate(estimateId) {
  try {
    ...
    // If we're in mock mode, return a dummy response
    if (process.env.ZOHO_MOCK_MODE === 'true') {
      console.log('Using mock mode for get estimate');
      return {
        estimate: {
          ...
          line_items: [
            {
              name: 'LABOR - Install new kitchen cabinets -',
              description: 'Item from PDF: LABOR - Install new kitchen cabinets -',
              rate: 0,
              quantity: 1
            },
            {
              name: 'MATERIALS - Kitchen cabinets -',
              description: 'Item from PDF: MATERIALS - Kitchen cabinets -',
              rate: 0,
              quantity: 1
            }
          ]
    ...
    const response = await axios.get(
      `${BASE_URL}/estimates/${estimateId}?organization_id=${process.env.ZOHO_ORGANIZATION_ID}`,
      {
        headers: {
          Authorization: `Zoho-oauthtoken ${accessToken}`,
          'Content-Type': 'application/json'
    ...
    console.log('Successfully retrieved estimate from Zoho Books');
    return response.data;
  } catch (error) {
    console.error('Error getting estimate from Zoho:', error.response?.data || error.message);

    // If we're in development mode, return a dummy response
    if (process.env.NODE_ENV === 'development') {
      console.log('Using dummy response for get estimate (DEVELOPMENT MODE)');
      return {
        estimate: {
          estimate_id: estimateId,
          estimate_number: 'EST-00001',
    ...
    throw new Error(`Failed to get estimate from Zoho: ${error.message}`);
  }
}

module.exports = { getAccessToken, createEstimateInZoho, attachPDFToEstimate, getEstimate, BASE_URL };
