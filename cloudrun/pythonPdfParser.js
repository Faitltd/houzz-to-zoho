...
async function parsePdfWithPython(pdfBuffer) {
  return new Promise((resolve, reject) => {
    try {
      ...
      let pythonScriptPath = '/app/parse_pdf_for_node.py';

      // For local testing, check if we're running outside of Docker
      if (!fs.existsSync(pythonScriptPath)) {
        // Try local path
        pythonScriptPath = path.join(__dirname, 'parse_pdf_for_node.py');
